...
std::vector<BBox> Utils::PostProcess(const string &path, aclmdlDataset *modelOutput, ImageData imageData)
{
    ...
    for (uint32_t i = 0; i < totalBox; i++) {
        ...
        {
            float class_prob = detectData[i*85 + LABEL + j];
            if (Maxclass < class_prob)
            {
                Maxclass = class_prob;
                Maxclass_Loc = j;
...
void Utils::ImageNchw(shared_ptr<ImageDesc>& imageData, std::vector<cv::Mat>& nhwcImageChs, uint32_t size) {
    ...
    while (i< 10)
    {
        std::cout<<(nhwcImageChs[0].ptr<float>(0))[640+i-1]<<std::endl;
        i++;
    }
...
bool Utils::PreProcess(shared_ptr<ImageDesc>& imageData, const string& imageFile) {
    ...
    */
    ...
    uint32_t size = RGB_IMAGE_SIZE_F32(MODEL_INPUT_WIDTH, MODEL_INPUT_HEIGHT, 3);
...
//    int i=0;
//    u_int8_t *pInt = (u_int8_t *)(imageData->data.get());
//    //std::shared_ptr<int> p4(new int(5));
//    //int *pInt = p4.get();
//    while (i< 10)
//    {
//        std::cout<<(int)pInt[i]<<std::endl;
//        //std::cout<<((uint8_t *)imageDev)[i]<<std::endl;
//        i++;
//    }
...
void Utils::DrawBoundBoxToImage(vector<BBox>& detectionResults, const string& origImagePath) {
    ...
    // string outpath(origImagePath.substr(pos));

    stringstream sstream;
    sstream.str("");
    sstream << "../res/out_" << filename;
...
int Utils::ReadImageFile(ImageData& image, std::string fileName)
{
    ...
    std::ifstream binFile(fileName, std::ifstream::binary);   // 以二进制模式打开文件
    if (binFile.is_open() == false) {
        ERROR_LOG("open file %s failed", fileName.c_str());
        return FAILED;
    }

// 将文件指针移动到文件末尾，并通过 tellg 获取文件大小。如果文件为空（binFileBufferLen == 0），则记录错误日志并返回失败。
...
// 重新将文件指针移动到文件开头, 动态分配一个缓冲区以存储文件内容, 读取文件内容到缓冲区并关闭文件
...
//---------------//
    ...
    acldvppJpegGetImageInfo(binFileBufferData, binFileBufferLen,
                            &(image.width), &(image.height), &ch);  // 获取图像的宽度、高度和通道数
