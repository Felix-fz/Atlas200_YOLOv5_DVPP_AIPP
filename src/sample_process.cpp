...
namespace {
    ...
    const char* kModelPath = "../model/yolov5s_v6.om";
}
...
Result SampleProcess::MainProcess(string input_path)
{
    ...
    ImageData image;
    // traversal files for preprocess, inference, and then post process
    for (string imageFile : file_vec) {
        count++;
        clock_t start, finish;
        double duration = 0.0;
        start = clock();
        Utils::ReadImageFile(image, imageFile);  // dvp解码-> image
        ...
        start = clock();
        // post process.
        vector<BBox> boxlists = Utils::PostProcess(imageFile, inferenceOutput, image);
        if (boxlists.empty()) {
            ERROR_LOG("pull model output data failed");
            continue;
            //return FAILED;
        ...
        Utils::DrawBoundBoxToImage(boxlists, imageFile);
        //Utils::WriteBoundBoxToTXT(boxlists, path);
    }
    avg_pre /= count;
    avg_forward /= count;
    avg_post /= count;
    INFO_LOG("Average preprocess cost: %.6f ms", avg_pre*1000);
    INFO_LOG("Average forward cost: %.6f ms", avg_forward*1000);
    INFO_LOG("Average postprocess cost: %.6f ms", avg_post*1000);
    INFO_LOG("Average total cost: %.6f ms", (avg_pre+avg_forward+avg_post)*1000);
    INFO_LOG("FPS: %.6f ", 1/(avg_pre+avg_forward+avg_post));
    return SUCCESS;
}
